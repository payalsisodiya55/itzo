...
import { Loader2, ArrowLeft, Download, FileText, User, Building2, Send, Paperclip, CheckCircle, XCircle, AlertCircle, MessageSquare, X } from 'lucide-react';
...
export default function AdminReportDetails() {
    ...
    const updateStatus = async (status) => {
        ...
        try {
            await axiosInstance.put(`/hrms/daily-reports/admin/${id}/status`, { status });
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-6rem)] max-w-[1400px] mx-auto p-4 sm:p-6">
            ...
            <div className="w-full lg:w-7/12 xl:w-2/3 flex flex-col bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden h-full">
                <div className="px-6 py-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-4 bg-slate-50 shrink-0">
                    <div className="flex items-center gap-3">
                        <button onClick={() => navigate('/ecs/hrms/reports/all')} className="p-1.5 -ml-1.5 rounded-lg text-slate-400 hover:bg-slate-200 hover:text-slate-700 transition-colors">
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                        <div>
                            <h2 className="text-base font-bold text-slate-800">
                                {report.employeeId?.adminId?.name}'s Report 
                            </h2>
                    ...
                    <div className="flex items-center gap-2">
                        {isPending && (
                            <>
                                <button onClick={() => updateStatus('Revision Requested')} disabled={statusUpdating} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors flex items-center gap-1 disabled:opacity-50">
                                    <AlertCircle className="w-3.5 h-3.5" /> Request Revision
                                </button>
                                <button onClick={() => updateStatus('Rejected')} disabled={statusUpdating} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors flex items-center gap-1 disabled:opacity-50">
                                    <XCircle className="w-3.5 h-3.5" /> Reject
                                </button>
                                <button onClick={() => updateStatus('Approved')} disabled={statusUpdating} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition-colors flex items-center gap-1 disabled:opacity-50">
                        ...
                        {!isPending && (
                            <span className={`text-[11px] font-bold px-3 py-1 rounded-full uppercase border ${
                                report.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' :
                                report.status === 'Rejected' ? 'bg-red-50 text-red-600 border-red-200' :
                                report.status === 'Revision Requested' ? 'bg-amber-50 text-amber-600 border-amber-200' :
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-white [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-orange-200 [&::-webkit-scrollbar-thumb]:rounded-full">
                    {comments.length === 0 ? (
                        ...
                        comments.map((msg, index) => {
                            const isMe = msg.senderType === 'Admin' || msg.senderType === 'Manager';
                            return (
                                    ...
                                    <div className={`flex items-end gap-2 max-w-[90%] sm:max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                        ...
                                        <div className={`flex flex-col gap-1 ${isMe ? 'items-end' : 'items-start'}`}>
                                            <span className="text-[10px] font-medium text-slate-400 px-1">
                                                {isMe ? 'You' : 'Employee'} • {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                            </span>
